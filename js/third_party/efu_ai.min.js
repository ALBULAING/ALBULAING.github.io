...
class efuAI {
  constructor() {
      ...
      this.root = this.config.apiUrl; // ChatGPT API endpoint
      this.aiTalkMode = false;
      this.aiPostExplanation = '';
      this.scoGPTIsRunning = false;
      this.apiKey = this.config.apiKey;//"sk-kP59Dy3oFrmwO6YXAf869eA4A8B94749916868E6A4B84869"; // Replace with your OpenAI API key
  }
  ...
  async fetch(content) {
      const messages = [
          { role: "user", content: `假如你是一个文章摘要助手，请为以下内容生成摘要：${content}` }
      ];
