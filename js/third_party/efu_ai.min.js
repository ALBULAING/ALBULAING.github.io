...
class efuAI {
  constructor() {
      const { config } = GLOBAL_CONFIG.post_ai; // 从配置中获取config
      this.root = 'https://free.gpt.ge/v1/chat/completions' ; // 获取API端点
      this.apiKey = config.apiKey; // 获取API密钥
      this.model = config.model; // 获取模型
      this.maxTokens = config.max_tokens; // 获取最大token数
  ...
  async fetch(content) {
      ...
      const response = await fetch(this.root, {
          ...
          body: JSON.stringify({
              model: this.model, // You can change the model if needed
              messages: messages,
              max_tokens: this.maxTokens // Adjust the max tokens as needed
          })
      });
